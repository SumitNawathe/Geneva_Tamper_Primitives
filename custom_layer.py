...
class CustomLayer(Layer):
    ...
    def __init__(self, layer, header_string):
        ...
        self.getters = {
            'load' : self.get_load,
            'options' : None # self.get_options
        }
        self.setters = {
            'load' : self.set_load,
            'options' : None # self.set_options
        }


        self.generators = {name:(lambda: random.randint(0, 2**size - 1)) for (name, size) in ans}
        self.fields = [name for (name, size) in ans]
